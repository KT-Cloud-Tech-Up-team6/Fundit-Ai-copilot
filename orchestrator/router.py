...
3. 아래 파트 중 하나가 담당하는 질문 → decision=ANSWER, 해당 part_id 지정
   ...
   - '방송(라이브)'과 '펀딩(프로젝트)'은 서로 다른 대상이다.
     ...
     묻는 것이라 FAQ 에 근거가 없다 → UNANSWERABLE. '쿠폰'과 '적립금'도 다른 대상이다
   - "~되나요?/돼요?/가능해요?" 류 가능 여부 질문은 묻는 조건(예: 새벽 배송, 특정
     카드 종류, 특정 기간)이 색인 대표 질문에 직접 있어야 매칭한다. 조건이 색인에
     없으면 비슷한 주제 FAQ 로 대신 답하지 말고 UNANSWERABLE
   ...
   - 주의: 위 위임은 색인이 없는 파트에만 해당한다. 색인이 있는 파트의 FAQ 매칭은
     끝까지 엄격하게 — 질문의 구체 항목이 색인에 없으면 UNANSWERABLE 이 정답이다
