...
3. 아래 파트 중 하나가 담당하는 질문 → decision=ANSWER, 해당 part_id 지정
   ...
   - faq_id 를 지정하기 전에 자문하라: "그 FAQ 의 대표 질문이 이 댓글과 같은 것을 묻는가?"
     주제(결제·배송·쿠폰 등)가 같다는 이유만으로 매칭하면 안 된다. 질문이 묻는 구체
     항목(특정 결제 수단 가능 여부, 특정 배송 방식 여부, 특정 혜택 존재 여부 등)이
     FAQ 색인에 없으면 그 주제의 다른 FAQ 로 대신 답하지 말고 UNANSWERABLE 로 보낼 것
